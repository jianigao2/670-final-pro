...
turtles-own [my-patch my-xcor my-ycor energy happiness whole-happiness value happy? days-unhappy own-stop-fishing]
...
to setup
  ...
  ask patches [
  ifelse distancexy 0 0 < 14
    [ set pcolor blue ]
    [set pcolor grey]
  ]

  if monitors?
  [ask n-of 5 patches with [pcolor = grey]
      [sprout-monitors 1
...
to go
  ...
  [ask monitors
    [work-monitor
      ...
      change-back]]
  tick
  ifelse ((count (fishermen) + count (monitors)  > 4)  and (count carps + count salmons + count tilapias) > 0 )
  [
  set hour hour + 1
...
to walk
  ask monitors[
    let this-patch one-of possible-walk-moves
...
to-report possible-walk-moves
  report patches in-radius 6 with [ pcolor = grey ]
end
...
to search
  if hour = 1
  [set happiness 0]
  if  hour >= 32 and hour <= 39[
    ifelse any? fishermen in-radius 6[
      let that-guy one-of fishermen in-radius 6
      if ([happiness] of that-guy) > stop-fishing[
        set happiness happiness + ([happiness] of that-guy)
        ask that-guy [set happiness 0 set color green]]
  ][walk]
  ]
end

to increase-monitors
  let total-initial-fish ((prob-carp + prob-salmon + prob-tilapia) * count (patches with [pcolor = blue]))
  ...
  if remaining-fish-ratio < 0.4 or unhappy-ratio > 0.7 [
...
to work
   if hour = 1 [
    ...
    ifelse random-float 1 < 0.3
       [  ifelse color = green [set color black ]
          [set own-stop-fishing stop-fishing + 15
          set color pink]
         ]
      [set color black]
  ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count (fishermen)"
"pen-1" 1.0 0 -8053223 true "" "plot count (monitors)"
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="monitors?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold-of-happiness">
      <value value="22"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-number-monitors">
      <value value="16"/>
    </enumeratedValueSet>
